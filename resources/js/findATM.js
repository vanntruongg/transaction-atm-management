...
async function initMap() {
    fetch("http://127.0.0.1:8000/getdataInit")
        ...
        .then((response) => {
            ...
            const htmlDataBank = dataBank
                .map((bank) => {
                    return `<option value="${bank.NH_Ma}">${bank.NH_Ten}</option>`;
                })
                .join("");

            const htmlDataService = dichVu
                .map((dv) => {
...
async function handleSearchATM() {
    ...
    const URL =
        range == 0
            ? `http://127.0.0.1:8000/getListBankAccept/${selectService}/${atmSelected}`
            : `http://127.0.0.1:8000/getListBankAccept/${selectService}/${atmSelected}/${range}`;
                ...
                fetch(URLATMOFBANK)
                    ...
                    .then((response) => {
                        ...
                        response.forEach((item) => {
                            ...
                            L.marker([latATM, lngATM], { icon: atmIcon })
                                .addTo(map)
                                .bindPopup(idATM)
                                .openPopup();
